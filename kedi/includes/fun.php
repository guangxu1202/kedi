...
function form_fields($id,$path='',$edit='false'){
	...
	$str.=$opt;	
	$str.="<tr><td style=\"width:30%\"></td><td style=\"width:70%\"><input type=\"hidden\" value=\"{$lang}\" name=\"lang\"/><input type=\"hidden\" value=\"{$f_id}\" name=\"f_id\"/><input type=\"submit\" name=\"submit\" class=\"pt_go\" value=\"提交\"/><input type=\"reset\" name=\"reset\" class=\"pt_reset\" value=\"重置\"/></td></tr>";
	$str.="</table></form>";
	}
	return $str;
}
...
function get_sitemap_child($id){
	global $category;
	$sql="select * from ".DB_PRE."category where cate_parent=".$id." and cate_hide=0 order by cate_order desc";
	$child=$GLOBALS['mysql']->fetch_asc($sql);
	$path=CMS_SELF;
	$rel=array();
...
function get_child_cate($parent_id,$lang,$is_content=0,$cateid=''){
	...
	foreach($rel as $row){
		if(!$row['cate_hide']){
			...
			$child[$row['id']]['cate_pic1']=empty($cate_info['cate_pic1'])?CMS_SELF.'upload/no_pc.gif':CMS_SELF.'upload/'.$row['cate_pic1'];
			$child[$row['id']]['cate_pic2']=empty($cate_info['cate_pic2'])?CMS_SELF.'upload/no_pc.gif':CMS_SELF.'upload/'.$row['cate_pic2'];
			$child[$row['id']]['cate_pic3']=empty($cate_info['cate_pic3'])?CMS_SELF.'upload/no_pc.gif':CMS_SELF.'upload/'.$row['cate_pic3'];
			$childnum++;
			$child[$row['id']]['childnum']=$childnum;
