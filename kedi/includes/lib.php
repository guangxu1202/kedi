...
function list_article(){
	...
	$offset = ($page-1) * $row;
	$rel = $GLOBALS['mysql'] -> fetch_asc("select m.*,f.* from ".DB_PRE."maintb as m left join ".DB_PRE.$table." as f on m.id=f.id where m.lang='".$lang."' and m.id=f.id and category in (".$list_cate.") order by m.top desc,m.updatetime desc limit ".$offset.",".$page_size);
	$path = CMS_SELF;
	if(!empty($rel))
	{
		$i = 1;
...
if(!empty($_confing)){
 foreach($_confing as $k=>$v){
	...
	if($GLOBALS['mysql']->fetch_rows($sql)){
		$prev_rel=$GLOBALS['mysql']->fetch_asc("select id,title,title_color,title_style,url,is_html,is_url,url_add,c_order,custom_url,addtime from ".DB_PRE."maintb where category={$cat_id} and id>{$cate_id} order by id asc limit 0,1");
		//处理标题
		$font_style='';
	...
	$rel[0]['next']='';
	if($GLOBALS['mysql']->fetch_rows($sql)){
		$next_rel=$GLOBALS['mysql']->fetch_asc("select id,title,title_color,title_style,url,is_html,is_url,url_add,c_order,custom_url,addtime from ".DB_PRE."maintb where category={$cat_id} and id<{$cate_id} order by id desc limit 0,1");
		//处理标题
		$font_style='';
...
function body_pages(){
	...
	if($_confing['web_html'][0])
	{
		...
		$url = empty($rel[0]['custom_url'])?CMS_SELF.$cate_info['cate_fold_name'].'/'.date('YmdHms',$rel[0]['addtime']):CMS_SELF.$cate_info['cate_fold_name'].'/'.$rel[0]['custom_url'];	
		/*
		if(!empty($rel[0]['url']))
		{
 ...
 */
function album($name=''){
	global $id,$channel_table,$arc_id;
	$arr=array();
 ...
 * @return  array
 */
function get_link($type=0,$str){
	global $lang;
	$str = $str;
	if(empty($lang)){return;}
	$type=empty($type)?0:$type;
	$sql="select link_name,link_url,link_logo from ".DB_PRE."link where lang='{$lang}' and link_type={$type} and link_mail='{$str}' order by link_order desc";
	$rel=$GLOBALS['mysql']->fetch_asc($sql);
	$rel_arr=array();
	if(!empty($rel)){
		foreach($rel as $k=>$v){
...
function nav_middle(){
	...
	if(!empty($cate_list)){
		...
		if(in_array('2',$cate_nav)){
			...
			$inum++;
			$rel[$k]['class']='hsub only'.' n'.$inum;
			if($v['id']==5){$rel[$k]['class']='hsub'.' n'.$inum;}
			if($parent_id==$v['id']){
				$rel[$k]['class']='hsub only'.' selected n'.$inum;
				if($v['id']==5){$rel[$k]['class']='hsub'.' selected n'.$inum;}
			}
...
function get_sitemap(){
	...
	$_confing=get_confing($GLOBALS['lang']);
	if(empty($GLOBALS['lang'])){return;}
	$parent=$GLOBALS['mysql']->fetch_asc("select*from ".DB_PRE."category where cate_parent=0 and lang='".$GLOBALS['lang']."' and cate_hide=0 order by cate_order desc");
	$path=CMS_SELF;
	$rel=array();
	if(!empty($parent)){
 ...
 * @return  array
 */
function get_cate_content($cate_id='',$limit='0,5',$order_type='updatetime',$filter='',$pic='no',$order='desc',$lang=''){
	...
	$sql.=(empty($lang))?" and m.lang='".$GLOBALS['lang']."'":" and m.lang='".$lang."'";
	$sql.=(in_array($order_type,array('id','hits','addtime','updatetime')))?" GROUP BY m.id order by m.top desc, m.".$order_type:" GROUP BY m.id order by m.top desc, m.updatetime ";
	$sql.=(in_array($order,array('asc','desc')))?' '.$order:' desc';
	$sql.=empty($limit)?' limit 0,5':" limit ".$limit;
	$rel=$GLOBALS['mysql']->fetch_asc($sql);
	if(!empty($rel)){
 ...
 */
 ...
 function get_form($form_id=''){
 	global $lang;
 	if(empty($form_id)){
		$sql="select form_id from ".DB_PRE."mx_form where id=".intval($GLOBALS['id']);
		$rel=$GLOBALS['mysql']->fetch_asc($sql);
 ...
 */
 function get_tpl_cate_content($tpl_id='',$limit='0,5',$order_type='id',$filter='',$pic='no',$order='desc',$lang=''){
	...
	if($cate_rel[0]['cate_tpl']==2){
		...
		$cate_url=($cate_rel[0]['cate_html']&&$_confing['web_html'][0])?CMS_SELF.$cate_rel[0]['cate_fold_name'].'.html':$path.$list_php.'?id='.$cate_rel[0]['id'];
			if($c_v['cate_tpl']==3){
				$cate_url=($cate_rel[0]['cate_html']&&$_confing['web_html'][0])?CMS_SELF.$cate_rel[0]['cate_fold_name'].'.html':$path.$list_php.'?id='.$cate_rel[0]['id'];
			}
	}
	...
	if(!empty($cate_rel)){
		foreach($cate_rel as $c_v){
			...
			if($c_v['cate_tpl']==2){
				...
				$cate_url=($c_v['cate_html']&&$_confing['web_html'][0])?CMS_SELF.$c_v['cate_fold_name'].'.html':$path.$list_php.'?id='.$c_v['id'];
				if($c_v['cate_tpl']==3){
					$cate_url=($c_v['cate_html']&&$_confing['web_html'][0])?CMS_SELF.$c_v['cate_fold_name'].'.html':$path.$list_php.'?id='.$c_v['id'];
				}
			}
...
function get_tpl_list_nav($tpl_id='',$is_content=0){
	...
	if(!empty($cate_list)){
			foreach($cate_list as $k=>$v){
				...
				//开启内容获取选中栏目的推荐内容
			if($is_content){
					$content_sql = "SELECT*FROM ".DB_PRE."maintb WHERE category=".$v['id']." ORDER BY id DESC";
				...
				$nav_arr[$k]['cate_name']=$v['cate_name'];
				$nav_arr[$k]['cate_pic1']=empty($cate_info['cate_pic1'])?CMS_SELF.'upload/no_pc.gif':CMS_SELF.'upload/'.$cate_info['cate_pic1'];
				$nav_arr[$k]['cate_pic2']=empty($cate_info['cate_pic2'])?CMS_SELF.'upload/no_pc.gif':CMS_SELF.'upload/'.$cate_info['cate_pic2'];
				$nav_arr[$k]['cate_pic3']=empty($cate_info['cate_pic3'])?CMS_SELF.'upload/no_pc.gif':CMS_SELF.'upload/'.$cate_info['cate_pic3'];
 ...
 */
 ...
 function get_form2($form_id=''){
	...
	if(!empty($rel)){
		...
		//$return['php_file'] = CMS_SELF.'order/order_save.php';
	}
 ...
 */
 function get_content_key($index=0){
	...
	if(!empty($arr)){
		foreach($arr as $k=>$v){
			$rel[$k]['name'] = $v;
			$rel[$k]['url'] = CMS_SELF.'search/search.php?lang='.$GLOBALS['lang'].'&key='.$v;
		}
	}
	return $rel;
 }
 ...
 */
 function get_custom_nav($nav_show=0){
    ...
    /**
 * 根据栏目ID值获取栏目信息和栏目相关内容，可以使用多个栏目ID，只能获取主表的内容
 *
 * @access  public
