...
if($php_file_copy!='beescms3.0'){echo '<div style="height:20px; text-align:center">powerd by <a href="http://www.beescms.com">BEESCMS</a></div>';}//防止文件覆盖
...
function up_img($file,$size,$type,$thumb=0,$thumb_width='',$thumb_height='',$logo=1){
		...
		$up_file_name=$pic_name['filename'];
...
function page($url='',$page,$query='',$totalnum,$totalpages,$cate_id='',$ishtml=0){
	...
	if($php_file_copy!='beescms3.0'||$GLOBALS['tpl']->beescms_copyright!='BEESCMS3.0'){echo '<div style="height:20px; text-align:center">powerd by <a href="http://www.beescms.com">BEESCMS</a></div>';}//防止文件覆盖
	$str="";
	$op="";
...
function creat_html($htm_file,$err=''){
	...
	if($php_file_copy!='beescms3.0'||$GLOBALS['tpl']->beescms_copyright!='BEESCMS3.0'){$htm_data=$htm_datt.'<div style="height:20px; text-align:center">powerd by <a href="http://www.beescms.com">BEESCMS</a></div>';}//防止文件覆盖
	ob_clean();
	$fp=fopen($htm_file,'w');
	flock($fp,LOCK_EX);
...
function creat_html_file($htm_file,$data=''){
	global $php_file_copy;
	if($php_file_copy!='beescms3.0'||$GLOBALS['tpl']->beescms_copyright!='BEESCMS3.0'){$data=$data.'<div style="height:20px; text-align:center">powerd by <a href="http://www.beescms.com">BEESCMS</a></div>';}//防止文件覆盖
	$fp=fopen($htm_file,'w');
	flock($fp,LOCK_EX);
	if(!fwrite($fp,$data)){
		echo $err;
...
function get_position($parent,$cate,$path,$list_php=''){
	...
	if($php_file_copy!='beescms3.0'||$GLOBALS['tpl']->beescms_copyright!='BEESCMS3.0'){$str=$str.'<div style="height:20px; text-align:center">powerd by <a href="http://www.beescms.com">BEESCMS</a></div>';}//防止文件覆盖
	echo $str;
}
	...
	foreach($rel as $row){
		if(!$row['cate_hide']){
			...
			if($row['cate_tpl']==2){
				...
				$url=($row['cate_html']&&$_confing['web_html'][0])?CMS_SELF.$row['cate_fold_name'].'.html':CMS_SELF.$list_php.'?id='.$row['id'];
			if($row['cate_tpl']==3){
				$url=($row['cate_html']&&$_confing['web_html'][0])?CMS_SELF.$row['cate_fold_name'].'.html':CMS_SELF.$list_php.'?id='.$row['id'];
			}
			}
